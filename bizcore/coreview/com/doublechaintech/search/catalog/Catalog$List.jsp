...
<c:if test="${ empty catalogList}" >
	<div class="row" style="font-size: 30px;">
		<div class="col-xs-12 col-md-12" style="padding-left:20px">
		 ${userContext.localeMap['@not_found']}${userContext.localeMap['catalog']}! 
		 <a href="./${ownerBeanName}Manager/addCatalog/${result.id}/"><i class="fa fa-plus-square" aria-hidden="true"></i></a>
...
<%
 	...
 	SmartList list=(SmartList)request.getAttribute("catalogList"); 
...
%>
    ...
    <div class="row" style="font-size: 30px;">
		<div class="col-xs-12 col-md-12" style="padding-left:20px">
		<i class='fa fa-table'></i> ${userContext.localeMap['catalog']}(${totalCount})
		<a href="./${ownerBeanName}Manager/addCatalog/${result.id}/"><i class="fa fa-plus-square" aria-hidden="true"></i></a>
		 
		 		 	<div class="btn-group" role="group" aria-label="Button group with nested dropdown">		
	<c:forEach var="action" items="${result.actionList}">
		<c:if test="${'catalogList' eq action.actionGroup}">
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${catalogListName};${catalogListName}CurrentPage=${page.pageNumber};${catalogListName}RowsPerPage=${rowsPerPage}/' 
	...
	<table class="table table-striped" pageToken='catalogListCurrentPage=${currentPageNumber}'>
		<thead><tr>
...
<c:if test="${param.referName ne 'name'}">	<td contenteditable='true' class='edit-value'  propertyToChange='name' storedCellValue='${item.name}' prefix='${ownerBeanName}Manager/updateCatalog/${result.id}/${item.id}/'>${item.name}</td>
	...
	<td class="select_candidate_td"
			data-candidate-method="./catalogManager/requestCandidateSite/${ownerBeanName}/${item.id}/"
			data-switch-method="./catalogManager/transferToAnotherSite/${item.id}/"
			data-link-template="./siteManager/view/${'$'}{ID}/">
		...
		<div class="candidate_span" style="display:none;">
			<input type="text" data-provide="typeahead" class="input-sm form-control candidate-filter-input" autocomplete="off" />
				...
				<a href='#${ownerBeanName}Manager/copyCatalogFrom/${result.id}/${item.id}/' class='copy-action btn btn-success btn-xs'><i class="fa fa-files-o fa-lg"></i> ${userContext.localeMap['@copy']} </a>
