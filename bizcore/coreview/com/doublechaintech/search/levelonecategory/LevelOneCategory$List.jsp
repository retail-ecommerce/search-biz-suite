...
<c:if test="${ empty levelOneCategoryList}" >
 ...
 <c:if test="${not empty levelOneCategoryList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("levelOneCategoryList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${levelOneCategoryListName};${levelOneCategoryListName}CurrentPage=${page.pageNumber};${levelOneCategoryListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${levelOneCategoryList}">
				<tr currentVersion='${item.version}' id="levelOneCategory-${item.id}" ><td><a class="link-action-removed" href="./levelOneCategoryManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'catalog'}">
	<td class="select_candidate_td"
			data-candidate-method="./levelOneCategoryManager/requestCandidateCatalog/${ownerBeanName}/${item.id}/"
			data-switch-method="./levelOneCategoryManager/transferToAnotherCatalog/${item.id}/"
