...
public class ProductJDBCTemplateDAO extends SearchNamingServiceDAO implements ProductDAO{
	...
	*/
	...
	protected Product clone(AccessKey accessKey, Map<String,Object> options) throws Exception{
		...
		newProduct.setVersion(0);
 		...
 		if(isSaveProductRecommendationListEnabled(options)){
 			for(ProductRecommendation item: newProduct.getProductRecommendationList()){
 				item.setVersion(0);
 			}
 		}
	...
	protected Product extractProductRecommendationList(Product product, Map<String,Object> options){
		...
		SmartList<ProductRecommendation> productRecommendationList = getProductRecommendationDAO().findProductRecommendationByProduct(product.getId(),options);
		if(productRecommendationList != null){
			enhanceProductRecommendationList(productRecommendationList,options);
			product.setProductRecommendationList(productRecommendationList);
		}
	...
	protected Product analyzeProductRecommendationList(Product product, Map<String,Object> options){
		...
		SmartList<ProductRecommendation> productRecommendationList = product.getProductRecommendationList();
		if(productRecommendationList != null){
			getProductRecommendationDAO().analyzeProductRecommendationByProduct(productRecommendationList, product.getId(), options);
	...
	protected Product saveInternalProduct(Product product, Map<String,Object> options){
		...
		if(isSaveProductRecommendationListEnabled(options)){
	 		saveProductRecommendationList(product, options);
	 		//removeProductRecommendationList(product, options);
	...
	public Product planToRemoveProductRecommendationList(Product product, String productRecommendationIds[], Map<String,Object> options)throws Exception{
		...
		key.put(ProductRecommendation.PRODUCT_PROPERTY, product.getId());
	...
	protected Product saveProductRecommendationList(Product product, Map<String,Object> options){
	...
	protected Product removeProductRecommendationList(Product product, Map<String,Object> options){
	...
	public Product present(Product product,Map<String, Object> options){
		...
		presentProductRecommendationList(product,options);
	...
	//Using java8 feature to reduce the code significantly
 	protected Product presentProductRecommendationList(
			Product product,
			Map<String, Object> options) {

		SmartList<ProductRecommendation> productRecommendationList = product.getProductRecommendationList();		
				SmartList<ProductRecommendation> newList= presentSubList(product.getId(),
				productRecommendationList,
				options,
				getProductRecommendationDAO()::countProductRecommendationByProduct,
				getProductRecommendationDAO()::findProductRecommendationByProduct
				);

		
		product.setProductRecommendationList(newList);
