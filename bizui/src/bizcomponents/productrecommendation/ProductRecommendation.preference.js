...
import styles from './ProductRecommendation.preference.less'
...
const internalImageListOf = (productRecommendation) =>{
...
const internalSettingListOf = (productRecommendation) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ProductRecommendationService[candidateServiceName] 
...
const executeTrans = (productRecommendation,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=productRecommendation.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_productRecommendation/doJob",payload: payload})
...
const buildTransferModal = (productRecommendation,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(productRecommendation,targetComponent)}
...
const internalRenderExtraHeader = (productRecommendation) =>{
	return null
}
const internalRenderExtraFooter = (productRecommendation) =>{
	return null
}
...
<Description term="Product">{productRecommendation.product==null?appLocaleName(userContext,"NotAssigned"):productRecommendation.product.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Product","product","requestCandidateProduct",
	      "transferToAnotherProduct","anotherProductId",productRecommendation.product?productRecommendation.product.id:"")} 
...
const renderPermissionSetting = productRecommendation => {
  const {ProductRecommendationBase} = GlobalComponents
  return <PermissionSetting targetObject={productRecommendation}  targetObjectMeta={ProductRecommendationBase}/>


}


class ProductRecommendationPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Product Recommendation",cardsFor: "productRecommendation",cardsSource: this.props.productRecommendation,
  		subItems: [
...
}))(Form.create()(ProductRecommendationPreference))
