...
import styles from './ProductRecommendation.table.less'
...
class ProductRecommendationTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = ProductRecommendationBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/productRecommendation/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
