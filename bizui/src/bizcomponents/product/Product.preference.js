...
const internalSettingListOf = (product) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ProductService[candidateServiceName] 
...
const executeTrans = (product,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_product/doJob",payload: payload})
...
const buildTransferModal = (product,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(product,targetComponent)}
...
const internalRenderExtraHeader = (product) =>{
	return null
}
const internalRenderExtraFooter = (product) =>{
	return null
}
...
class ProductPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, productRecommendationCount, skuCount } = this.props.product
    const cardsData = {cardsName:"Product",cardsFor: "product",cardsSource: this.props.product,
  		subItems: [
