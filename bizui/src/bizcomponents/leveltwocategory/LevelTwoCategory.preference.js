...
const internalImageListOf = (levelTwoCategory) =>{
...
const internalSettingListOf = (levelTwoCategory) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = LevelTwoCategoryService[candidateServiceName] 
...
const executeTrans = (levelTwoCategory,targetComponent) =>{
...
const buildTransferModal = (levelTwoCategory,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(levelTwoCategory,targetComponent)}
...
const internalRenderExtraHeader = (levelTwoCategory) =>{
	return null
}
const internalRenderExtraFooter = (levelTwoCategory) =>{
	return null
}
...
<Description term="Parent Category">{levelTwoCategory.parentCategory==null?appLocaleName(userContext,"NotAssigned"):levelTwoCategory.parentCategory.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Parent Category","levelOneCategory","requestCandidateParentCategory",
	      "transferToAnotherParentCategory","anotherParentCategoryId",levelTwoCategory.parentCategory?levelTwoCategory.parentCategory.id:"")} 
...
class LevelTwoCategoryPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, levelNCategoryCount } = this.props.levelTwoCategory
    const cardsData = {cardsName:"Level Two Category",cardsFor: "levelTwoCategory",cardsSource: this.props.levelTwoCategory,
  		subItems: [
...
}))(Form.create()(LevelTwoCategoryPreference))
