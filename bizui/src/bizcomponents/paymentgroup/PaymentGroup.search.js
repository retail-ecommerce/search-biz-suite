...
const showAssociateDialog = (targetComponent) => {
  ...
  <div>
	...
	data={{paymentGroupList:selectedRows}} owner={owner}  
...
class PaymentGroupSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {PaymentGroupTable} = GlobalComponents;
    const {PaymentGroupSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <PaymentGroupSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,PaymentGroupModalTable,"paymentGroupIds")}
...
export default Form.create()(PaymentGroupSearch)
