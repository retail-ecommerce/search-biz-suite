...
import styles from './PaymentGroup.preference.less'
...
const internalImageListOf = (paymentGroup) =>{
...
const internalSettingListOf = (paymentGroup) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PaymentGroupService[candidateServiceName] 
...
const executeTrans = (paymentGroup,targetComponent) =>{
...
const buildTransferModal = (paymentGroup,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(paymentGroup,targetComponent)}
...
const internalRenderExtraHeader = (paymentGroup) =>{
	return null
}
const internalRenderExtraFooter = (paymentGroup) =>{
	return null
}
...
<Description term="User Order">{paymentGroup.userOrder==null?"未分配":paymentGroup.userOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"User Order","userOrder","requestCandidateUserOrder",
	      "transferToAnotherUserOrder","anotherUserOrderId",paymentGroup.userOrder?paymentGroup.userOrder.id:"")} 
...
const renderPermissionSetting = paymentGroup => {
  const {PaymentGroupBase} = GlobalComponents
  return <PermissionSetting targetObject={paymentGroup}  targetObjectMeta={PaymentGroupBase}/>


}


class PaymentGroupPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Payment Group",cardsFor: "paymentGroup",cardsSource: this.props.paymentGroup,
  		subItems: [
...
}))(Form.create()(PaymentGroupPreference))
