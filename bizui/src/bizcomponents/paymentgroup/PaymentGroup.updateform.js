...
import styles from './PaymentGroup.updateform.less'
import PaymentGroupBase from './PaymentGroup.base'
...
class PaymentGroupUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const {fieldLabels} = PaymentGroupBase
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const paymentGroupId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, paymentGroupId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updatePaymentGroup`,
          payload: {
            id: owner.id,
            type: 'paymentGroup',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextPaymentGroupUpdateRow`,
        payload: {
          id: owner.id,
          type: 'paymentGroup',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新Payment Group"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新Payment Group"
...
}))(Form.create()(PaymentGroupUpdateForm))
