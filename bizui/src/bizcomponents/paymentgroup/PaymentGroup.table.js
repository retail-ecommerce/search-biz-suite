...
import styles from './PaymentGroup.table.less'
...
class PaymentGroupTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = PaymentGroupBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/paymentGroup/${record.id}/dashboard`}>{'查看'}</Link>}
