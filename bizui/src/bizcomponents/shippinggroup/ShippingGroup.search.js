...
const showAssociateDialog = (targetComponent) => {
  ...
  <div>
	...
	data={{shippingGroupList:selectedRows}} owner={owner}  
...
class ShippingGroupSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {ShippingGroupTable} = GlobalComponents;
    const {ShippingGroupSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ShippingGroupSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,ShippingGroupModalTable,"shippingGroupIds")}
...
export default Form.create()(ShippingGroupSearch)
