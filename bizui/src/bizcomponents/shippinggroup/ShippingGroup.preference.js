...
import styles from './ShippingGroup.preference.less'
...
const internalImageListOf = (shippingGroup) =>{
...
const internalSettingListOf = (shippingGroup) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ShippingGroupService[candidateServiceName] 
...
const executeTrans = (shippingGroup,targetComponent) =>{
...
const buildTransferModal = (shippingGroup,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(shippingGroup,targetComponent)}
...
const internalRenderExtraHeader = (shippingGroup) =>{
	return null
}
const internalRenderExtraFooter = (shippingGroup) =>{
	return null
}
...
<Description term="User Order">{shippingGroup.userOrder==null?"未分配":shippingGroup.userOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"User Order","userOrder","requestCandidateUserOrder",
	      "transferToAnotherUserOrder","anotherUserOrderId",shippingGroup.userOrder?shippingGroup.userOrder.id:"")} 
...
const renderPermissionSetting = shippingGroup => {
  const {ShippingGroupBase} = GlobalComponents
  return <PermissionSetting targetObject={shippingGroup}  targetObjectMeta={ShippingGroupBase}/>


}


class ShippingGroupPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Shipping Group",cardsFor: "shippingGroup",cardsSource: this.props.shippingGroup,
  		subItems: [
...
}))(Form.create()(ShippingGroupPreference))
