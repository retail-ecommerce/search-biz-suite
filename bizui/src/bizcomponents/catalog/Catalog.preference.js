...
import styles from './Catalog.preference.less'
...
const internalImageListOf = (catalog) =>{
...
const internalSettingListOf = (catalog) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CatalogService[candidateServiceName] 
...
const executeTrans = (catalog,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_catalog/doJob",payload: payload})
...
const buildTransferModal = (catalog,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(catalog,targetComponent)}
...
const internalRenderExtraHeader = (catalog) =>{
	return null
}
const internalRenderExtraFooter = (catalog) =>{
	return null
}
...
<Description term="Site">{catalog.site==null?"未分配":catalog.site.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Site","site","requestCandidateSite",
	      "transferToAnotherSite","anotherSiteId",catalog.site?catalog.site.id:"")} 
...
class CatalogPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, levelOneCategoryCount, productCount } = this.props.catalog
    const cardsData = {cardsName:"Catalog",cardsFor: "catalog",cardsSource: this.props.catalog,
  		subItems: [
