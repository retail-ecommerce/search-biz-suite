...
class CatalogSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {CatalogTable} = GlobalComponents;
    const {CatalogSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        ...
        {showDeletionDialog(this,CatalogModalTable,"catalogIds")}
...
export default Form.create()(CatalogSearch)
