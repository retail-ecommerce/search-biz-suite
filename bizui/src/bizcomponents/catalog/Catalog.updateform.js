...
import styles from './Catalog.updateform.less'
...
class CatalogUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const catalogId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, catalogId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextCatalogUpdateRow`,
        payload: {
          id: owner.id,
          type: 'catalog',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新Catalog"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新Catalog"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.sellerId} {...formItemLayout}>
                  {getFieldDecorator('sellerId', {
                    initialValue: selectedRow.sellerId,
