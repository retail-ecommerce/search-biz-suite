...
import styles from './ManualAdjustment.preference.less'
...
const internalImageListOf = (manualAdjustment) =>{
...
const internalSettingListOf = (manualAdjustment) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ManualAdjustmentService[candidateServiceName] 
...
const executeTrans = (manualAdjustment,targetComponent) =>{
...
const buildTransferModal = (manualAdjustment,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(manualAdjustment,targetComponent)}
...
const internalRenderExtraHeader = (manualAdjustment) =>{
	return null
}
const internalRenderExtraFooter = (manualAdjustment) =>{
	return null
}
...
<Description term="User Order">{manualAdjustment.userOrder==null?"未分配":manualAdjustment.userOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"User Order","userOrder","requestCandidateUserOrder",
	      "transferToAnotherUserOrder","anotherUserOrderId",manualAdjustment.userOrder?manualAdjustment.userOrder.id:"")} 
...
const renderPermissionSetting = manualAdjustment => {
  const {ManualAdjustmentBase} = GlobalComponents
  return <PermissionSetting targetObject={manualAdjustment}  targetObjectMeta={ManualAdjustmentBase}/>


}


class ManualAdjustmentPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Manual Adjustment",cardsFor: "manualAdjustment",cardsSource: this.props.manualAdjustment,
  		subItems: [
...
}))(Form.create()(ManualAdjustmentPreference))
