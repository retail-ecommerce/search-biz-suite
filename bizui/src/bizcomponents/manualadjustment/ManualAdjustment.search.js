...
const showAssociateDialog = (targetComponent) => {
  ...
  const { UserOrderAssociateForm } = GlobalComponents
  ...
  <div>
	...
	visible={currentAssociateModal==='userOrder'} 
	data={{manualAdjustmentList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'userOrder')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'userOrder')}/> 
...
class ManualAdjustmentSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {ManualAdjustmentTable} = GlobalComponents;
    const {ManualAdjustmentSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ManualAdjustmentSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,ManualAdjustmentModalTable,"manualAdjustmentIds")}
...
export default Form.create()(ManualAdjustmentSearch)
