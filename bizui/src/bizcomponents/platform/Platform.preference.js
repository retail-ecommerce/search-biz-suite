...
import styles from './Platform.preference.less'
...
const internalImageListOf = (platform) =>{
...
const internalSettingListOf = (platform) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PlatformService[candidateServiceName] 
...
const executeTrans = (platform,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_platform/doJob",payload: payload})
...
const buildTransferModal = (platform,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(platform,targetComponent)}
...
const internalRenderExtraHeader = (platform) =>{
	return null
}
const internalRenderExtraFooter = (platform) =>{
	return null
}
...
class PlatformPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, siteCount, profileCount, userOrderCount } = this.props.platform
...
}))(Form.create()(PlatformPreference))
