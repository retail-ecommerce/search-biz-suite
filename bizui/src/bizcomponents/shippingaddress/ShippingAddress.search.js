...
class ShippingAddressSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {ShippingAddressTable} = GlobalComponents;
    const {ShippingAddressSearchForm} = GlobalComponents;
    const {ShippingAddressModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ShippingAddressSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,ShippingAddressModalTable,"shippingAddressIds")}
...
export default Form.create()(ShippingAddressSearch)
