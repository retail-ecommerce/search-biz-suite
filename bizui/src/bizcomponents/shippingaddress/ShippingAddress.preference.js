...
import styles from './ShippingAddress.preference.less'
...
const internalImageListOf = (shippingAddress) =>{
...
const internalSettingListOf = (shippingAddress) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ShippingAddressService[candidateServiceName] 
...
const executeTrans = (shippingAddress,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_shippingAddress/doJob",payload: payload})
...
const buildTransferModal = (shippingAddress,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(shippingAddress,targetComponent)}
...
const internalRenderExtraHeader = (shippingAddress) =>{
	return null
}
const internalRenderExtraFooter = (shippingAddress) =>{
	return null
}
...
<Description term="Line1">{shippingAddress.line1}</Description> 
<Description term="Line2">{shippingAddress.line2}</Description> 
...
const renderPermissionSetting = shippingAddress => {
  const {ShippingAddressBase} = GlobalComponents
  return <PermissionSetting targetObject={shippingAddress}  targetObjectMeta={ShippingAddressBase}/>


}


class ShippingAddressPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Shipping Address",cardsFor: "shippingAddress",cardsSource: this.props.shippingAddress,
  		subItems: [
...
}))(Form.create()(ShippingAddressPreference))
