...
const showAssociateDialog = (targetComponent) => {
  ...
  <div>
	...
	data={{orderPromotionList:selectedRows}} owner={owner}  
...
class OrderPromotionSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {OrderPromotionTable} = GlobalComponents;
    const {OrderPromotionSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <OrderPromotionSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,OrderPromotionModalTable,"orderPromotionIds")}
...
export default Form.create()(OrderPromotionSearch)
