...
import styles from './OrderPromotion.preference.less'
...
const internalImageListOf = (orderPromotion) =>{
...
const internalSettingListOf = (orderPromotion) =>{
...
const internalLargeTextOf = (orderPromotion) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = OrderPromotionService[candidateServiceName] 
...
const executeTrans = (orderPromotion,targetComponent) =>{
...
const buildTransferModal = (orderPromotion,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(orderPromotion,targetComponent)}
...
const internalRenderExtraHeader = (orderPromotion) =>{
	return null
}
const internalRenderExtraFooter = (orderPromotion) =>{
	return null
}
...
const internalSummaryOf = (orderPromotion,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="Id">{orderPromotion.id}</Description> 
<Description term="Name">{orderPromotion.name}</Description> 
...
<Description term="User Order">{orderPromotion.userOrder==null?"未分配":orderPromotion.userOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"User Order","userOrder","requestCandidateUserOrder",
	      "transferToAnotherUserOrder","anotherUserOrderId",orderPromotion.userOrder?orderPromotion.userOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(orderPromotion,targetComponent)}
      </DescriptionList>
	)
...
const renderPermissionSetting = orderPromotion => {
  const {OrderPromotionBase} = GlobalComponents
  return <PermissionSetting targetObject={orderPromotion}  targetObjectMeta={OrderPromotionBase}/>


}


class OrderPromotionPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.orderPromotion
    const cardsData = {cardsName:"Order Promotion",cardsFor: "orderPromotion",cardsSource: this.props.orderPromotion,
  		subItems: [
...
export default connect(state => ({
  orderPromotion: state._orderPromotion,
}))(Form.create()(OrderPromotionPreference))
