...
import styles from './UserOrder.preference.less'
...
const internalImageListOf = (userOrder) =>{
...
const internalSettingListOf = (userOrder) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = UserOrderService[candidateServiceName] 
...
const executeTrans = (userOrder,targetComponent) =>{
...
const buildTransferModal = (userOrder,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(userOrder,targetComponent)}
...
const internalRenderExtraHeader = (userOrder) =>{
	return null
}
const internalRenderExtraFooter = (userOrder) =>{
	return null
}
...
<Description term="User">{userOrder.user==null?"未分配":userOrder.user.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"User","profile","requestCandidateUser",
	      "transferToAnotherUser","anotherUserId",userOrder.user?userOrder.user.id:"")} 
...
const renderPermissionSetting = userOrder => {
  const {UserOrderBase} = GlobalComponents
  return <PermissionSetting targetObject={userOrder}  targetObjectMeta={UserOrderBase}/>


}


class UserOrderPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, lineItemCount, orderPromotionCount, manualAdjustmentCount, shippingGroupCount, paymentGroupCount } = this.props.userOrder
    const cardsData = {cardsName:"User Order",cardsFor: "userOrder",cardsSource: this.props.userOrder,
  		subItems: [
...
}))(Form.create()(UserOrderPreference))
