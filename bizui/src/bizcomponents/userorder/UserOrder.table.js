...
import styles from './UserOrder.table.less'
...
class UserOrderTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = UserOrderBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/userOrder/${record.id}/dashboard`}>{'查看'}</Link>}
