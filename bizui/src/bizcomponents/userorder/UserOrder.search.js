...
const showAssociateDialog = (targetComponent) => {
  ...
  <div>
	...
	data={{userOrderList:selectedRows}} owner={owner}  
...
class UserOrderSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {UserOrderTable} = GlobalComponents;
    const {UserOrderSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <UserOrderSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,UserOrderModalTable,"userOrderIds")}
...
export default Form.create()(UserOrderSearch)
