...
import styles from './PaymentMethod.preference.less'
...
const internalImageListOf = (paymentMethod) =>{
...
const internalSettingListOf = (paymentMethod) =>{
	const userContext = null
	const optionList = [ 
	]
...
const internalLargeTextOf = (paymentMethod) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PaymentMethodService[candidateServiceName] 
...
const executeTrans = (paymentMethod,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_paymentMethod/doJob",payload: payload})
...
const buildTransferModal = (paymentMethod,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(paymentMethod,targetComponent)}
...
const internalRenderExtraHeader = (paymentMethod) =>{
	return null
}
const internalRenderExtraFooter = (paymentMethod) =>{
	return null
}
...
const internalSummaryOf = (paymentMethod,targetComponent) =>{
    const userContext = null
	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="Id">{paymentMethod.id}</Description> 
<Description term="Name">{paymentMethod.name}</Description> 
<Description term="Number">{paymentMethod.number}</Description> 
<Description term="Line1">{paymentMethod.line1}</Description> 
<Description term="Line2">{paymentMethod.line2}</Description> 
<Description term="City">{paymentMethod.city}</Description> 
<Description term="Expire On">{ moment(paymentMethod.expireOn).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(paymentMethod,targetComponent)}
      </DescriptionList>
	)
...
const renderPermissionSetting = paymentMethod => {
  const {PaymentMethodBase} = GlobalComponents
  return <PermissionSetting targetObject={paymentMethod}  targetObjectMeta={PaymentMethodBase}/>


}


class PaymentMethodPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.paymentMethod
    const cardsData = {cardsName:"Payment Method",cardsFor: "paymentMethod",cardsSource: this.props.paymentMethod,
  		subItems: [
...
export default connect(state => ({
  paymentMethod: state._paymentMethod,
}))(Form.create()(PaymentMethodPreference))
