...
import styles from './PaymentMethod.table.less'
...
class PaymentMethodTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = PaymentMethodBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/paymentMethod/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
