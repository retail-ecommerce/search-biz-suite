...
import styles from './LineItem.preference.less'
...
const internalImageListOf = (lineItem) =>{
...
const internalSettingListOf = (lineItem) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = LineItemService[candidateServiceName] 
...
const executeTrans = (lineItem,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_lineItem/doJob",payload: payload})
...
const buildTransferModal = (lineItem,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(lineItem,targetComponent)}
...
const internalRenderExtraHeader = (lineItem) =>{
	return null
}
const internalRenderExtraFooter = (lineItem) =>{
	return null
}
...
<Description term="User Order">{lineItem.userOrder==null?"未分配":lineItem.userOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"User Order","userOrder","requestCandidateUserOrder",
	      "transferToAnotherUserOrder","anotherUserOrderId",lineItem.userOrder?lineItem.userOrder.id:"")} 
...
const renderPermissionSetting = lineItem => {
  const {LineItemBase} = GlobalComponents
  return <PermissionSetting targetObject={lineItem}  targetObjectMeta={LineItemBase}/>


}


class LineItemPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Line Item",cardsFor: "lineItem",cardsSource: this.props.lineItem,
  		subItems: [
...
}))(Form.create()(LineItemPreference))
