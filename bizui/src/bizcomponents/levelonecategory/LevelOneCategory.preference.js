...
import styles from './LevelOneCategory.preference.less'
...
const internalImageListOf = (levelOneCategory) =>{
...
const internalSettingListOf = (levelOneCategory) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = LevelOneCategoryService[candidateServiceName] 
...
const executeTrans = (levelOneCategory,targetComponent) =>{
...
const buildTransferModal = (levelOneCategory,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(levelOneCategory,targetComponent)}
...
const internalRenderExtraFooter = (levelOneCategory) =>{
	return null
}
...
<Description term="Catalog">{levelOneCategory.catalog==null?appLocaleName(userContext,"NotAssigned"):levelOneCategory.catalog.displayName}
...
class LevelOneCategoryPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Level One Category",cardsFor: "levelOneCategory",cardsSource: this.props.levelOneCategory,
  		subItems: [
...
}))(Form.create()(LevelOneCategoryPreference))
