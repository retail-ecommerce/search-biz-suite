...
import styles from './LevelNCategory.preference.less'
...
const internalImageListOf = (levelNCategory) =>{
...
const internalSettingListOf = (levelNCategory) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = LevelNCategoryService[candidateServiceName] 
...
const executeTrans = (levelNCategory,targetComponent) =>{
...
const buildTransferModal = (levelNCategory,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(levelNCategory,targetComponent)}
...
const internalRenderExtraHeader = (levelNCategory) =>{
	return null
}
const internalRenderExtraFooter = (levelNCategory) =>{
	return null
}
...
<Description term="Parent Category">{levelNCategory.parentCategory==null?"未分配":levelNCategory.parentCategory.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Parent Category","levelTwoCategory","requestCandidateParentCategory",
...
const renderPermissionSetting = levelNCategory => {
  const {LevelNCategoryBase} = GlobalComponents
  return <PermissionSetting targetObject={levelNCategory}  targetObjectMeta={LevelNCategoryBase}/>


}


class LevelNCategoryPreference extends Component {
...
}))(Form.create()(LevelNCategoryPreference))
