...
import styles from './Profile.preference.less'
...
const internalImageListOf = (profile) =>{
...
const internalSettingListOf = (profile) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ProfileService[candidateServiceName] 
...
const executeTrans = (profile,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_profile/doJob",payload: payload})
...
const buildTransferModal = (profile,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(profile,targetComponent)}
...
const internalRenderExtraHeader = (profile) =>{
	return null
}
const internalRenderExtraFooter = (profile) =>{
	return null
}
...
<Description term="Last Update Time">{profile.lastUpdateTime}</Description> 
...
class ProfilePreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, userOrderCount } = this.props.profile
