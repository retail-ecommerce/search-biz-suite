...
export default {
  ...
  effects: {
    ...
    *addUserOrder({ payload }, { call, put }) {
      ...
      const data = yield call(ProfileService.addUserOrder, id, parameters)
      if (hasError(data)) {
      ...
      const location = { pathname: `/profile/${id}/list/\UserOrderList/User Order列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateUserOrder({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(ProfileService.updateUserOrder, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/profile/${id}/list/\UserOrderList/User Order列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *removeUserOrderList({ payload }, { call, put }) {
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(ProfileService.removeUserOrderList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
