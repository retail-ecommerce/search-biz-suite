...
import styles from './Brand.preference.less'
...
const internalImageListOf = (brand) =>{
...
const internalSettingListOf = (brand) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BrandService[candidateServiceName] 
...
const executeTrans = (brand,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_brand/doJob",payload: payload})
...
const buildTransferModal = (brand,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(brand,targetComponent)}
...
const internalRenderExtraHeader = (brand) =>{
	return null
}
const internalRenderExtraFooter = (brand) =>{
	return null
}
...
const renderPermissionSetting = brand => {
  const {BrandBase} = GlobalComponents
  return <PermissionSetting targetObject={brand}  targetObjectMeta={BrandBase}/>


}


class BrandPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, productCount, productRecommendationCount } = this.props.brand
