...
class BrandSearch extends PureComponent {
  ...
  render(){
    ...
    const {BrandTable} = GlobalComponents;
    const {BrandSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        ...
        {showDeletionDialog(this,BrandModalTable,"brandIds")}
...
export default Form.create()(BrandSearch)
