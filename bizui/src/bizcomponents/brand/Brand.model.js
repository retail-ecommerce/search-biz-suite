...
export default {
  ...
  effects: {
    ...
    *addProductRecommendation({ payload }, { call, put }) {
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(BrandService.addProductRecommendation, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/brand/${id}/list/\ProductRecommendationList/Product Recommendation+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateProductRecommendation({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      console.log('get form parameters', parameters)
      const data = yield call(BrandService.updateProductRecommendation, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/brand/${id}/list/\ProductRecommendationList/Product Recommendation列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *removeProductRecommendationList({ payload }, { call, put }) {
     const userContext = null
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(BrandService.removeProductRecommendationList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
