...
import styles from './Site.updateform.less'
...
class SiteUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, siteId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextSiteUpdateRow`,
        payload: {
          id: owner.id,
          type: 'site',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新Site"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新Site"
