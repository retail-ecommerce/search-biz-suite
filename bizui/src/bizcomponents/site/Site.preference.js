...
import styles from './Site.preference.less'
...
const internalImageListOf = (site) =>{
...
const internalSettingListOf = (site) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = SiteService[candidateServiceName] 
...
const executeTrans = (site,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_site/doJob",payload: payload})
...
const buildTransferModal = (site,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(site,targetComponent)}
...
const internalRenderExtraHeader = (site) =>{
	return null
}
const internalRenderExtraFooter = (site) =>{
	return null
}
...
<Description term="Last Update Time">{site.lastUpdateTime}</Description> 
...
const renderPermissionSetting = site => {
  const {SiteBase} = GlobalComponents
  return <PermissionSetting targetObject={site}  targetObjectMeta={SiteBase}/>
...
class SitePreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, catalogCount } = this.props.site
    const cardsData = {cardsName:"Site",cardsFor: "site",cardsSource: this.props.site,
  		subItems: [
